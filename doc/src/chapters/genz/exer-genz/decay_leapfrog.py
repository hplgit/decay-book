...
def analyze(u):
    ...
    R_step1 = (u(t+dt) - u(t))/dt + sym.Rational(1,2)*(
...
def u_discrete_exact():
    ...
    error_A1 = sym.exp(-p).series(p, 0, 4) - A1.series(p, 0, 4)
    #error_A1 = sym.simplify(error_A1)
    error_A2 = sym.exp(-p).series(p, 0, 4) - A2.series(p, 0, 4)
    #error_A2 = sym.simplify(error_A2)
    ...
    q = sym.solve([C1 + C2 - 1, C1*A1 + C2*A2 - 1*sym.exp(-p*1)], [C1, C2])
    print 'Exact Leapfrog solution:'
    solution = q[C1]*A1 + q[C2]*A2
    print solution
    print sym.printing.latex(solution)
